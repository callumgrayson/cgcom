...
function Reset({ handler }) {
  return (
    <button type="button" onClick={handler} className="fib-grid-reset-button">
      Reset
    </button>
  );
}
...
function FibGrid() {
  ...
  return (
    <div className="fib-grid-FibGrid">
      <div className="fib-grid-col-box">
        <div className="fib-grid-col-2">
          <Reset handler={handleReset} />
          {grid.current &&
            grid.current.map((row, idxRow) => (
              <div key={idxRow} className="fib-grid-row">
